...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .header-stripe {
    height: 4px;
    background: linear-gradient(90deg, var(--green-600), #4ADE80, var(--green-600));
  }
  .header-inner {
  ...
  .nav-tab.active { color: #fff; border-bottom-color: #4ADE80; }
  ...
  .mobile-nav-btn {
    ...
    color: var(--gray-400); padding: 10px 4px 12px;
  ...
  .mobile-nav-btn::after {
    ...
    background: var(--green-600); transform: scaleX(0);
  ...
  @media (max-width: 640px) {
    ...
    .header-badge { padding: 6px 10px; min-width: 56px; }
    .header-badge-val { font-size: 17px; }
  }
  ...
  @media (max-width: 640px) {
    ...
    .page-content { padding: 18px 0 95px; }
  }
  ...
  .card-hover:active { transform: translateY(0); box-shadow: var(--shadow-sm); }
  .card + .card { margin-top: 12px; }

  /* Card com borda colorida lateral */
  .card-accent-green { border-left: 4px solid var(--green-600); }
  .card-accent-amber { border-left: 4px solid var(--amber-700); }
  .card-accent-red   { border-left: 4px solid var(--red-700); }
  ...
  .stat-value {
    ...
    line-height: 1.1; margin-top: 6px; color: var(--gray-900);
  }
  ...
  .field-input:focus {
    ...
    box-shadow: 0 0 0 3px rgba(33,149,106,0.15);
  }
  ...
  .btn-primary {
    ...
    box-shadow: 0 2px 8px rgba(26,122,80,0.3);
  }
  .btn-primary:hover { background: var(--green-800); box-shadow: 0 4px 16px rgba(26,122,80,0.4); }
  ...
  .btn-ghost:hover { background: var(--gray-50); color: var(--gray-900); border-color: var(--gray-400); }
  ...
  /* ── BARRA DE PROGRESSO ── */
  .progress-track {
    background: var(--gray-100); border-radius: 99px; height: 10px; overflow: hidden;
  }
  ...
  .search-input:focus {
    border-color: var(--green-600);
    box-shadow: 0 0 0 3px rgba(33,149,106,0.12);
  }
  ...
  .filter-pill.active { background: var(--green-700); color: #fff; border-color: var(--green-700); box-shadow: 0 2px 8px rgba(26,122,80,0.25); }
  ...
  .ben-needs {
    margin-top: 12px; font-size: 13.5px; color: var(--gray-600);
    background: var(--gray-50); padding: 10px 14px; border-radius: var(--radius-xs);
    border-left: 3px solid var(--green-200, #BBF7D0);
    line-height: 1.5;
  }
  ...
  @media (max-width: 640px) {
    .summary-banner { padding: 20px; border-radius: var(--radius); }
  ...
  /* ── FORM CARD (seção de formulário com número) ── */
  .form-section {
    background: var(--white); border-radius: var(--radius);
  ...
  .form-step-title {
    font-family: var(--font-sans); font-size: 15px; font-weight: 700; color: var(--gray-800);
  }
...
function Inp({ label, required, style: s, ...p }) {
  return (
    <div className="field">
...
function gerarRelatorio(tipo, dados) {
  ...
  if (tipo === "beneficiarios") {
    body = `<h2 style="margin:0 0 18px;font-size:18px;font-weight:800">Beneficiários cadastrados — ${beneficiarios.length} famílias</h2>
      <table width="100%" cellspacing="0" style="border-collapse:collapse;border:1px solid #E5E7EB;border-radius:8px;overflow:hidden">
    ...
    body = `<h2 style="margin:0 0 18px;font-size:18px;font-weight:800">Registro de atendimentos — ${atendimentos.length} registros</h2>
    ...
    body = `<h2 style="margin:0 0 8px;font-size:18px;font-weight:800">Histórico de doações — ${movs.length} registros</h2>
      <p style="margin:0 0 18px;color:#6B7280;font-size:14px">Total de entradas: <strong>${ent}</strong> &nbsp;·&nbsp; Total de saídas: <strong>${sai}</strong></p>
      <table width="100%" cellspacing="0" style="border-collapse:collapse;border:1px solid #E5E7EB">
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Relatório · Sistema Social Recife</title>
    <style>*{box-sizing:border-box}body{font-family:Georgia,serif;max-width:960px;margin:0 auto;padding:40px 32px;color:#111827}h1,h2{font-family:system-ui,sans-serif}@media print{.no-print{display:none}}</style></head>
    <body>
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:32px;padding-bottom:20px;border-bottom:3px solid #155E3E">
        <div>
          <h1 style="margin:0;color:#155E3E;font-size:26px;font-weight:800">🤝 Sistema Social · Recife</h1>
          <p style="margin:8px 0 0;font-size:14px;color:#6B7280">Relatório gerado em ${now}</p>
        </div>
        <button class="no-print" onclick="window.print()" style="background:#155E3E;color:#fff;border:none;padding:13px 26px;border-radius:10px;cursor:pointer;font-size:14px;font-weight:700;box-shadow:0 2px 8px rgba(0,0,0,0.15)">🖨 Imprimir / Salvar PDF</button>
      </div>
      ${body}
    </body></html>`;
...
function BeneficiariosModule({ beneficiarios, setBeneficiarios, atendimentos }) {
    ...
    <div>
      ...
      <div className="card" style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        <div className="form-grid-2">
          <Inp label="Nome completo" required value={form.nome} onChange={f("nome")} placeholder="Ex: Maria da Silva" />
        ...
        <Inp label="Endereço completo" value={form.endereco} onChange={f("endereco")} placeholder="Rua, número, complemento" />
        <div className="form-grid-2">
      ...
      <div>
        ...
        <div className="card card-accent-green" style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
            <div>
              <h3 style={{ fontFamily: "var(--font-sans)", fontSize: 24, fontWeight: 800, letterSpacing: "-0.02em", color: "var(--gray-900)" }}>{b.nome}</h3>
          ...
          <div className="detail-grid">
            {b.cpf && <div className="detail-item"><p className="detail-key">CPF</p><p className="detail-val">{b.cpf}</p></div>}
            {b.telefone && <div className="detail-item"><p className="detail-key">Telefone</p><p className="detail-val">{b.telefone}</p></div>}
            {b.bairro && <div className="detail-item"><p className="detail-key">Bairro</p><p className="detail-val">{b.bairro}</p></div>}
            {b.endereco && <div className="detail-item"><p className="detail-key">Endereço</p><p className="detail-val">{b.endereco}</p></div>}
            {b.numPessoas && <div className="detail-item"><p className="detail-key">Pessoas na família</p><p className="detail-val">{b.numPessoas} pessoa{b.numPessoas !== "1" ? "s" : ""}</p></div>}
            {b.renda && <div className="detail-item"><p className="detail-key">Renda familiar</p><p className="detail-val">{fCur(b.renda)}</p></div>}
          </div>
          ...
          : hists.map(a => (
            <div key={a.id} className="card" style={{ marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <span style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 14, color: "var(--gray-800)" }}>📅 {fDate(a.data)}</span>
  ...
  return (
    <div>
      ...
      {filtered.length === 0
        ? <Empty icon="👥" title="Nenhum beneficiário encontrado" sub={beneficiarios.length === 0 ? "Clique em '+ Novo cadastro' para começar a registrar as famílias atendidas." : "Tente buscar por outro nome ou bairro."} />
        : filtered.map(b => (
  ...
  return (
    <div>
      ...
      {tab === "estoque" && <>
        ...
        {estoque.length === 0
          ? <Empty icon="📦" title="Estoque vazio" sub="Registre uma doação recebida na aba 'Registrar entrada' para começar." />
                ...
                return (
                  <div key={item.id} className="card" style={{ marginBottom: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: meta ? 14 : 0 }}>
                      <div>
                        <p style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 16, color: "var(--gray-900)" }}>{item.nome}</p>
                    ...
                    {meta && <>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                        ...
                        <span style={{ fontFamily: "var(--font-sans)", fontSize: 12, fontWeight: 700, color: item.quantidade >= meta.meta ? "var(--green-700)" : "var(--amber-700)" }}>
      ...
      {tab === "entrada" && (
        <div className="card">
          <h3 style={{ fontFamily: "var(--font-sans)", fontSize: 18, fontWeight: 800, marginBottom: 22, color: "var(--gray-900)" }}>Registrar doação recebida</h3>
      ...
      {tab === "metas" && <>
        ...
        {metas.length === 0
          ? <Empty icon="🎯" title="Nenhuma meta definida" sub="Defina metas mensais de arrecadação acima para acompanhar o progresso." />
                ...
                return (
                  <div key={m.id} className="card" style={{ marginBottom: 12 }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
                      {pct < 100
                        ? <p style={{ fontSize: 13, color: "var(--amber-700)", fontWeight: 600 }}>Faltam {m.meta - atual} {m.unidade} para atingir a meta</p>
      ...
      {tab === "historico" && <>
        ...
        {movs.length === 0
          ? <Empty icon="📋" title="Nenhuma movimentação" sub="O histórico de entradas e saídas aparecerá aqui." />
          : movs.slice(0, 60).map(m => (
            <div key={m.id} className="card" style={{ marginBottom: 10, display: "flex", alignItems: "flex-start", gap: 16 }}>
              <div className="mov-icon" style={{ background: m.tipo === "entrada" ? "var(--green-100)" : "var(--amber-100)" }}>
                {m.tipo === "entrada" ? "⬇" : "⬆"}
              </div>
              <div style={{ flex: 1 }}>
                <p style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 14, color: "var(--gray-900)" }}>
                  {m.itemNome} <span style={{ fontWeight: 500, color: "var(--gray-500)" }}>— {m.quantidade} {m.unidade}</span>
                </p>
    ...
    <div>
          ...
          : <>
            ...
            {busca && (
              <div className="autocomplete-list">
                ...
                {benFiltrados.length === 0 && <p style={{ padding: "14px 18px", fontSize: 14, color: "var(--gray-400)" }}>Nenhuma família encontrada com esse nome.</p>}
              </div>
            )}
            {beneficiarios.length === 0 && (
              <div className="alert alert-amber" style={{ marginTop: 10, marginBottom: 0 }}>
                <span className="alert-icon">⚠️</span>
                <p style={{ fontSize: 13, color: "var(--amber-700)" }}>Nenhuma família cadastrada. Vá até a aba <strong>Beneficiários</strong> e faça o cadastro primeiro.</p>
              </div>
            )}
          </>}
      </FormSection>
      ...
      <FormSection step="2" title="Itens a distribuir">
        {itens.map((it, i) => (
          <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 100px auto", gap: 12, marginBottom: 12, alignItems: "end" }}>
            ...
            {itens.length > 1 && (
              <button onClick={() => setItens(itens.filter((_, idx) => idx !== i))}
                style={{ background: "var(--red-50)", border: "1px solid var(--red-100)", cursor: "pointer", color: "var(--red-700)", fontSize: 14, padding: "11px 14px", borderRadius: "var(--radius-xs)", alignSelf: "flex-end", fontWeight: 700 }}>✕</button>
            )}
          </div>
        ))}
      ...
      <FormSection step="3" title="Informações do atendimento">
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <Txta label="Observações" value={obs} onChange={e => setObs(e.target.value)} placeholder="Situação da família, condições do atendimento..." />
        </div>
      </FormSection>
        ...
        <div className="filter-row">
          {["todos", ...mesesDisp].map(m => (
            <button key={m} className={`filter-pill${filtroMes === m ? " active" : ""}`} onClick={() => setFiltroMes(m)}>
              {m === "todos" ? "Todos os meses" : ymLabel(m)}
            </button>
          ))}
        </div>
      )}
        ...
        : atFilt.map(a => (
          <div key={a.id} className="card card-accent-purple" style={{ marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              <div>
                <p style={{ fontFamily: "var(--font-sans)", fontWeight: 700, fontSize: 16, color: "var(--gray-900)" }}>{a.beneficiarioNome || "Beneficiário não identificado"}</p>
                <p style={{ fontSize: 13, color: "var(--gray-400)", marginTop: 3 }}>
                  📅 {fDate(a.data)}{a.voluntario && `  ·  👤 ${a.voluntario}`}
                </p>
              </div>
  ...
  return (
    <div>
      ...
      <div className="summary-banner">
        ...
        <p style={{ fontFamily: "var(--font-sans)", fontSize: 26, fontWeight: 800, letterSpacing: "-0.03em" }}>🤝 Sistema Social · Recife</p>
        <div className="summary-stats">
          {[[beneficiarios.length, "Famílias"], [totalPessoas, "Pessoas"], [totalDist, "Itens distribuídos"], [doadores, "Doadores"]].map(([v, l]) => (
            <div key={l} className="summary-stat">
              <div className="summary-stat-val">{v}</div>
              <div className="summary-stat-label">{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
        {[
          ["beneficiarios", "👥", "Lista de beneficiários", `${beneficiarios.length} famílias cadastradas com nome, bairro, renda e necessidades`],
          ["atendimentos", "📝", "Registro de atendimentos", `${atendimentos.length} atendimentos com itens entregues, datas e voluntários`],
          ["estoque", "📦", "Histórico de doações", `${movs.length} movimentações com entradas e saídas detalhadas`],
        ].map(([tipo, icon, titulo, desc]) => (
          <div key={tipo} className="rel-export-card">
            <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1 }}>
              <span className="rel-export-icon">{icon}</span>
              <div>
                <p className="rel-export-title">{titulo}</p>
                <p className="rel-export-desc">{desc}</p>
              </div>
            </div>
      ...
      <h3 style={{ fontFamily: "var(--font-sans)", fontSize: 17, fontWeight: 700, marginBottom: 14 }}>📊 Atividade do mês — {ymLabel(mes)}</h3>
      ...
      {beneficiarios.length === 0
        ? <Empty icon="🗺" title="Nenhum dado disponível" sub="Os dados por bairro aparecerão conforme as famílias forem cadastradas." />
        : [...new Set(beneficiarios.map(b => b.bairro).filter(Boolean))].sort().map(bairro => {
          ...
          return (
            <div key={bairro} style={{ marginBottom: 14 }}>
      ...
      <header className="app-header">
        ...
        <div className="header-inner">
          ...
          <div className="header-text-wrap">
            ...
            <div className="header-subtitle">Dados salvos localmente · funciona sem internet</div>
          </div>
        ...
        <div className="desktop-nav-inner">
          {NAV.map(n => (
            <button key={n.id} className={`nav-tab${mod === n.id ? " active" : ""}`} onClick={() => setMod(n.id)}>
              <span className="nav-tab-icon">{n.icon}</span> {n.label}
            </button>
          ))}
        </div>
      </nav>

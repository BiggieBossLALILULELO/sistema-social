...
function gerarRelatorioFamilia(entidade, familias) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Cadastro de Família</title><style>body{font-family:Arial,sans-serif;max-width:1100px;margin:0 auto;padding:30px;color:#111}@media print{.no-print{display:none}}</style></head><body>
    ...
    <table width="100%" style="border-collapse:collapse;border:1px solid #E5E7EB">
      ...
      ${familias.map((f, i) => `<tr><td style="${td};text-align:center;font-weight:700">${i + 1}</td><td style="${td}"><strong>${f.chefeNome || "—"}</strong><br/><span style="color:#6B7280">CPF: ${f.chefeCPF || "—"}</span></td><td style="${td}">${f.maeNome || "—"}</td><td style="${td}">${f.endereco || "—"}</td><td style="${td}">${f.nis || "—"}</td><td style="${td}">${f.dataRegistro ? new Date(f.dataRegistro).toLocaleDateString("pt-BR") : "—"}</td><td style="${td};text-align:center">${/^\d+$/.test(String(f.faixa1 || "").trim()) ? f.faixa1 : ""}</td><td style="${td};text-align:center">${/^\d+$/.test(String(f.faixa2 || "").trim()) ? f.faixa2 : ""}</td><td style="${td};text-align:center">${/^\d+$/.test(String(f.faixa3 || "").trim()) ? f.faixa3 : ""}</td><td style="${td};text-align:center">${/^\d+$/.test(String(f.faixa4 || "").trim()) ? f.faixa4 : ""}</td><td style="${td};text-align:center">${/^\d+$/.test(String(f.faixa5 || "").trim()) ? f.faixa5 : ""}</td></tr>`).join("")}
    </table>
